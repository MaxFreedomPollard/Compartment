...
def test_starter_pack_ships_and_verifies():
    ...
    assert len(records) == vectors.shape[0] == header["records"] == 6100
...
def test_seed_lands_in_main_fully_editable(vault):
    ...
    assert vault.db.count("main") == 6100
    ...
    assert vault.db.count("main") == 6099
...
def test_legacy_starter_vault_merges_on_unlock(tmp_path):
    ...
    assert v3.db.count("main") == 6099 and v3.db.count("packs/starter") == 0
